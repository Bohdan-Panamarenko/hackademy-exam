...
type UserRepository interface {
	Add(string, *UserToDoStorage) error
	Get(string) (*UserToDoStorage, error)
	Update(string, *UserToDoStorage) error
	Delete(string) (*UserToDoStorage, error)
}
...
func (u *UserService) Register(w http.ResponseWriter, r *http.Request) {
	...
	err = u.repository.Add(newUser.Email, NewUserToDoStorageWithUser(newUser))
	if err != nil {
		HandleError(err, w)
		return
	}
